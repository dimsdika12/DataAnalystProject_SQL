...
WITH SelectedColumns AS (
    ...
    WHERE location in ('Brunei','Cambodia','Indonesia','Laos','Malaysia','Myanmar','Philiphines','Singapore','Thailand','Timor','Vietnam')
)
...
-- Identifies countries with the highest death count compared to their population
SELECT location,population, MAX(total_deaths) as DeathsCount, (MAX(total_deaths)/population)*100 as DeathsRate 
FROM SelectedColumns
GROUP BY location,population
ORDER BY 4 DESC
...
WITH CovidVaccine AS (
    ...
    WHERE d.location in ('Brunei','Cambodia','Indonesia','Laos','Malaysia','Myanmar','Philiphines','Singapore','Thailand','Timor','Vietnam')
)
